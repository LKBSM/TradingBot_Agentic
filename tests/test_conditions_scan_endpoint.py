...
def test_scan_returns_full_match_with_met_conditions():
    ...
    # The other 5 combos have no reading → reported as unavailable, never invented.
    assert len(body["unavailable"]) == 5


def test_scan_reports_partial_match_transparently():
    readings = {
        ("XAUUSD", "M15"): _reading(
            "XAUUSD", "M15", mtf={"h4": "bullish", "h1": "bullish", "m15": "bullish"}
        )  # no OB at price
    }
