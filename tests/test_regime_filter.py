...
def test_ny_boundaries():
    """NY window is [13:00, 21:00) — boundary semantics matter."""
    f = RegimeFilter(skip_ny=True, vol_pctl_max=None)
...
def test_ny_drops_first_even_if_vol_low(ny_ts):
    """Session gate evaluates before vol gate; NY drop reason wins."""
...
def test_stats_track_drops(london_ts, ny_ts):
    f = RegimeFilter(skip_ny=True, vol_pctl_max=0.75)
    f.evaluate(london_ts, None)               # allowed
    f.evaluate(ny_ts, None)                   # dropped (NY)
...
def test_from_env_defaults():
    with patch.dict(os.environ, {}, clear=False):
        for k in ("REGIME_FILTER_SKIP_NY", "REGIME_FILTER_VOL_PCTL_MAX"):
