...
# mtf_aligned now composes each timeframe's OWN regime.trend, supplied by the
# scan route as instrument_trends ({"M15"/"H1"/"H4": trend}) — NOT the
# per-reading mtf_confluence (which structurally can never hold all three).
...
def test_mtf_aligned_unmet_when_divergent():
    trends = {"M15": "bearish", "H1": "neutral", "H4": "bullish"}
    res = evaluate_condition(_reading(), {"type": "mtf_aligned", "direction": "any"}, trends)
    assert res["met"] is False
    assert res["available"] is True  # judged, not a data gap
...
def test_mtf_aligned_unavailable_when_a_timeframe_is_missing():
    trends = {"M15": "bullish", "H1": "bullish"}  # H4 reading missing
    res = evaluate_condition(_reading(), {"type": "mtf_aligned", "direction": "any"}, trends)
    assert res["met"] is False
    assert res["available"] is False  # data gap, not a real "non"
...
def test_evaluate_reading_and_logic_full_match():
    ...
    assert out["context"]["mtf_trends"] == {"h4": "bullish", "h1": "bullish", "m15": "bullish"}


def test_evaluate_reading_and_logic_partial_is_not_matched():
    r = _reading()  # no OB at price
    out = evaluate_reading(
        r,
...
def test_evaluate_reading_or_logic_matches_on_any():
    out = evaluate_reading(_reading(), [{"type": "mtf_aligned"}, {"type": "price_in_ob"}], "OR", _ALIGNED_BULL)
    assert out["matched"] is True
    assert out["met_count"] == 1
