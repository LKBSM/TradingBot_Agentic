...
async def access_me(
    ...
    # when not enforced. When ON, only an authenticated account with an active
    # subscription (or the owner) has access.
    if not gate_on:
        has_access = True
    ...
    return {
        ...
        # An authenticated account that is logged in but not entitled — the
        # "account page + subscribe invitation" state (PAY-1).
        "subscription_required": gate_on and account is not None and not has_access,
    }
